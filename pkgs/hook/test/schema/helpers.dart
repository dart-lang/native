...
void testField({
  ...
  test('$schemaUri $fieldPath missing', () {
    ...
    dynamic dataToModify = dataDecoded;
    final fieldsToNavigate = [...field];
    while (fieldsToNavigate.length > 1) {
      dataToModify = dataToModify[fieldsToNavigate.removeAt(0)];
    }
    assert(fieldsToNavigate.length == 1);
    if (dataToModify is List) {
      final index = fieldsToNavigate[0] as int;
      dataToModify.removeAt(index);
    } else {
      dataToModify.remove(fieldsToNavigate[0]);
    }
  ...
  test('$schemaUri $fieldPath wrong type', () {
    ...
    assert(fieldsToNavigate.length == 1);
    final originalValue = dataToModify[fieldsToNavigate[0]];
    final wrongTypeValue = originalValue is int ? '123' : 123;
    dataToModify[fieldsToNavigate[0]] = wrongTypeValue;
