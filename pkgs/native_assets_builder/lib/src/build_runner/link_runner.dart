...
/// The programmatic API to be used by Dart launchers to invoke native linking.
///
/// These methods are invoked by launchers such as dartdev (for `dart run`)
/// and flutter_tools (for `flutter run` and `flutter build`).
class NativeAssetsLinkRunner {
  final Logger logger;
  final Uri dartExecutable;

  NativeAssetsLinkRunner({
  ...
  ///
  /// Completes the future with an error if the linking fails.
  ...
  Future<DryRunResult> dryRun({
    ...
    for (final package in plan) {
      ...
      final (packageAssets, _, _, packageSuccess) = await _linkPackage(
        config,
        packageLayout.packageConfigUri,
        workingDirectory,
        null,
  ...
  Future<_PackageBuildRecord> _linkPackageCached(
    BuildConfig config,
  ...
  Future<_PackageBuildRecord> _linkPackage(
    BuildConfig config,
    Uri packageConfigUri,
    Uri workingDirectory,
    Uri? resourceIdentifiers,
    ...
    final linkDotDart = config.packageRoot.resolve('link.dart');
    ...
    final arguments = [
      ...
      '--build-output=${outDir.resolve(BuildOutput.fileName).toFilePath()}',
      '--resource-identifiers=${resourceIdentifiers?.toFilePath()}',
      linkDotDart.toFilePath(),
...
build_output.yaml contained a format error.
      ...
      if (!success) {
        final linkOutputFile =
            File.fromUri(outDir.resolve(BuildOutput.fileName));
        if (await linkOutputFile.exists()) {
          await linkOutputFile.delete();
        }
      }
    }
  }
...
/// The result from a [NativeAssetsLinkRunner.dryRun].
abstract interface class DryRunResult {
  ...
  ///
  /// All error messages are streamed to [NativeAssetsLinkRunner.logger].
  bool get success;
}
...
/// The result from a [NativeAssetsLinkRunner.link].
abstract class LinkResult implements DryRunResult {
  ...
  /// [NativeAssetsLinkRunner] needs to be invoked again. The
  /// [NativeAssetsLinkRunner] determines per package with native assets
  /// if it needs to run the build again.
  List<Uri> get dependencies;
}

final class _LinkResultImpl implements LinkResult {
  @override
  final List<Asset> assets;
