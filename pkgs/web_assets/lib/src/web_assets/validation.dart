...
List<String> _validateHookInput(List<EncodedAsset> assets) {
  ...
  for (final asset in assets) {
    ...
    final webAsset = WebUriAsset.fromEncoded(asset);
    errors.addAll(
      _validateFile(
        'LinkInput.assets.web asset "${webAsset.id}" file',
        webAsset.file,
      ),
    );
  }
...
Future<ValidationErrors> validateWebAssetBuildOutput(
  BuildInput input,
  BuildOutput output,
) => _validateWebAssetBuildOrLinkOutput(input, [
...
Future<ValidationErrors> validateWebAssetLinkOutput(
  LinkInput input,
  LinkOutput output,
) => _validateWebAssetBuildOrLinkOutput(
  input,
  output.assets.encodedAssets,
  false,
);

Future<ValidationErrors> _validateWebAssetBuildOrLinkOutput(
  HookInput input,
