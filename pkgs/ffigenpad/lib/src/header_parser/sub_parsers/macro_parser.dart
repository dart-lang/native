...
List<Constant> parseSavedMacros() {
  ...
  Pointer<Pointer<Uint8>> clangCmdArgs = nullptr;
  var cmdLen = 0;
...
String _getWrittenRepresentation(String macroName, Pointer<Uint8> strPtr) {
  final sb = StringBuffer();
  try {
    ...
    // in the catch block.
    final result = strPtr.cast<Uint8>().toDartString();
    ...
    _logger.warning("Couldn't decode Macro string '$macroName' as Utf8, using "
        'ASCII instead.');
    sb.clear();
    // final length = strPtr.cast<Uint8>().length;
    final charList = strPtr.toCharList();
