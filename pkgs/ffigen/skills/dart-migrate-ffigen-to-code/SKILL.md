---
name: dart-migrate-ffigen-to-code
...
Make a temporary copy of every generated file so that the newly generated bindings can be diffed line-by-line:
```bash
...
Create the generator entrypoint at `tool/ffigen.dart` (the standard location across Dart ecosystem packages). Ensure the script is executable and resolves paths relative to `Platform.script` so that it can be invoked from any working directory:
...
Future<void> main() async {
  ...
  final generator = FfiGenerator(
    ...
    output: Output(
      dart: DartOutput(path: packageRoot.resolve('lib/src/generated_bindings.dart')),
    ),
  );
...
- Translate `headers` to `Input`.
- Translate `output`, `preamble`, `comments`, and `ffi-native` / `name` to `Output` and `BindingStyle`.
- Translate declaration filters, renames, leaf flags, and packing into `Visitor` callbacks.
- Translate `type-map` and `import.symbol-files` into `importType`.
- Translate `language: objc` and `external-versions` into `ObjectiveC`.
...
```
**Verification rules**:
- **Allowed diffs**: Trivial differences in comments (e.g. `// Generated by ffigen`), formatting of generator notices, or minor whitespace changes caused by `dart format`.
- **Forbidden diffs**: Any differences in public APIs, class names, method signatures, parameter names, struct fields, enum constants, native types, leaf annotations, packing annotations, or exposed addresses.
- If unintended diffs exist, adjust the `Visitor` filters or `Output` settings in `tool/ffigen.dart` and re-run until the diff is completely clean.
...
| *(Implicit format)* | N/A | `Output(format: true)` | Defaults to `true`. Automatically runs `dart format` on generated files. |
...
In YAML, filters and renames were configured with string matching and regex maps under declaration keys. In modern Dart, all filtering and renaming occurs inside the `visitors: [Visitor(...)]` list:
...
| `generate-for-package-objective-c` | `bool` | `@Deprecated` internal flag: `ObjectiveC(generateForPackageObjectiveC: true)`. Only used by `package:objective_c`. |
...
### 9. Modern Dart-Only Features

The modern Dart API provides several capabilities that were impossible or difficult in YAML:

1.  **Symbol Tree Shaking with `@RecordUse`**:
    To enable tree-shaking for compiled native assets:
    ```dart
    output: Output(
      dart: DartOutput(path: packageRoot.resolve('lib/src/bindings.g.dart')),
      recordUseMapping: packageRoot.resolve('lib/src/bindings.record_use_mapping.g.dart'),
    ),
    visitors: [
      Visitor(
        func: (node) {
          node.isIncluded = true;
          node.recordUse = true; // Annotates with @RecordUse()
        },
      ),
    ],
    ```
2.  **Experimental C++ Generation (`Cpp`)**:
    Generate bindings for C++ classes and member methods:
    ```dart
    FfiGenerator(
      cpp: const Cpp(),
      output: Output(
        dart: DartOutput(path: packageRoot.resolve('lib/src/cpp_bindings.dart')),
        cppFile: packageRoot.resolve('src/cpp_bindings.cpp'),
      ),
      visitors: [
        Visitor(
          cppClass: (node) => node.isIncluded = true,
          cppMethod: (node) => node.isIncluded = true,
        ),
      ],
    )
    ```
3.  **Arbitrary Procedural Transformation in Visitors**:
    You can inspect AST properties, log warnings, check collections, or alter names programmatically:
    ```dart
    final knownOverrides = {'init_with_foo': 'initFoo'};
    Visitor(
      func: (node) {
        if (knownOverrides.containsKey(node.originalName)) {
          node.name = knownOverrides[node.originalName]!;
        }
      },
    )
    ```
...
### Example 3: Objective-C Framework Bindings

#### BEFORE: `ffigen.yaml`
```yaml
output:
  bindings: 'lib/src/avf_audio.g.dart'
  objc-bindings: 'lib/src/avf_audio.g.m'
language: objc
headers:
  entry-points:
    - '/System/Library/Frameworks/AVFAudio.framework/Headers/AVAudioPlayer.h'
external-versions:
  ios:
    min: '12.0.0'
  macos:
...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    ...
    output: Output(
      dart: DartOutput(
        path: packageRoot.resolve('lib/src/avf_audio.g.dart'),
      ),
      objectiveCFile: packageRoot.resolve('lib/src/avf_audio.g.m'),
      preamble: '// AUTO-GENERATED FILE - DO NOT MODIFY.\n',
    ),
    visitors: [
      Visitor(
        objCInterface: (node) {
          if (node.originalName == 'AVAudioPlayer') {
            node.isIncluded = true;
          }
        },
        objCMethod: (node) {
          if (node.parent.originalName == 'AVAudioPlayer') {
            node.isIncluded = allowedMethods.contains(node.selector);
          }
        },
      ),
    ],
