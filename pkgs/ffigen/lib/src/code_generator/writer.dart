...
''');
  ...
  List<Binding> _getSymbolBindings() {
    final bindings = _allBindings;
    if (!canGenerateSymbolOutput) {
      throw Exception(
        'Invalid state: generateSymbolOutput() called before generate()',
      );
    }
...
const symbols = <String, ImportedType>{''');
    ...
    try {
      ...
      context.logger.severe('Failed to format generated symbols Dart file: $e');
      return raw;
    }
  }

  static String _defaultImportPrefix(String importPath) {
    if (importPath.startsWith('package:')) {
      final pkg = importPath.substring('package:'.length).split('/').first;
      final sanitized = pkg.replaceAll(RegExp(r'[^a-zA-Z0-9_]'), '_');
      if (sanitized.isNotEmpty && !sanitized.startsWith(RegExp(r'[0-9]'))) {
        return sanitized;
      }
    }
