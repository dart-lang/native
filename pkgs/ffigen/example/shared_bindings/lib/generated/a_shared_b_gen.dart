// AUTO GENERATED FILE, DO NOT EDIT.
//
// Generated by `package:ffigen`.
// ignore_for_file: type=lint, unused_import, unused_element, deprecated_member_use_from_same_package
import 'dart:ffi' as ffi;
import 'package:shared_bindings/generated/base_gen.dart' as shared_bindings;

/// Bindings to `headers/a.h` with shared definitions from `headers/base.h`.
class NativeLibraryASharedB {
  /// Holds the symbol lookup function.
  final ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName)
  _lookup;

  /// The symbols are looked up in [dynamicLibrary].
  NativeLibraryASharedB(ffi.DynamicLibrary dynamicLibrary)
    : _lookup = dynamicLibrary.lookup;

  /// The symbols are looked up with [lookup].
  NativeLibraryASharedB.fromLookup(
    ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName) lookup,
  ) : _lookup = lookup;

  void a_func1() {
    return _a_func1();
  }

  late final _a_func1Ptr = _lookup<ffi.NativeFunction<ffi.Void Function()>>(
    'a_func1',
  );
  late final _a_func1 = _a_func1Ptr.asFunction<void Function()>();

  void a_func2(
    shared_bindings.BaseStruct2 s,
    shared_bindings.BaseUnion2 u,
    shared_bindings.BaseTypedef2 t,
  ) {
    return _a_func2(s, u, t);
  }

  late final _a_func2Ptr =
      _lookup<
        ffi.NativeFunction<
          ffi.Void Function(
            shared_bindings.BaseStruct2,
            shared_bindings.BaseUnion2,
            shared_bindings.BaseTypedef2,
          )
        >
      >('a_func2');
  late final _a_func2 = _a_func2Ptr
      .asFunction<
        void Function(
          shared_bindings.BaseStruct2,
          shared_bindings.BaseUnion2,
          shared_bindings.BaseTypedef2,
        )
      >();

  void a_func3(shared_bindings.DartBaseNativeTypedef1 i) {
    return _a_func3(i);
  }

  late final _a_func3Ptr =
      _lookup<
        ffi.NativeFunction<
          ffi.Void Function(shared_bindings.BaseNativeTypedef1)
        >
      >('a_func3');
  late final _a_func3 = _a_func3Ptr
      .asFunction<void Function(shared_bindings.DartBaseNativeTypedef1)>();

  void a_func4(shared_bindings.DartBaseNativeTypedef1 i) {
    return _a_func4(i);
  }

  late final _a_func4Ptr =
      _lookup<
        ffi.NativeFunction<
          ffi.Void Function(shared_bindings.BaseNativeTypedef2)
        >
      >('a_func4');
  late final _a_func4 = _a_func4Ptr
      .asFunction<void Function(shared_bindings.DartBaseNativeTypedef1)>();

  void a_func5(shared_bindings.DartBaseNativeTypedef1 i) {
    return _a_func5(i);
  }

  late final _a_func5Ptr =
      _lookup<
        ffi.NativeFunction<
          ffi.Void Function(shared_bindings.BaseNativeTypedef3)
        >
      >('a_func5');
  late final _a_func5 = _a_func5Ptr
      .asFunction<void Function(shared_bindings.DartBaseNativeTypedef1)>();
}

enum A_Enum {
  A_ENUM_1(0),
  A_ENUM_2(1);

  final int value;
  const A_Enum(this.value);

  static A_Enum fromValue(int value) => switch (value) {
    0 => A_ENUM_1,
    1 => A_ENUM_2,
    _ => throw ArgumentError('Unknown value for A_Enum: $value'),
  };
}

const int A_MACRO_1 = 1;

final class A_Struct1 extends ffi.Struct {
  @ffi.Int()
  external int a;

  static ffi.Pointer<A_Struct1> $allocate(
    ffi.Allocator $allocator, {
    required int a,
  }) => $allocator<A_Struct1>()..ref.a = a;
}

final class A_Union1 extends ffi.Union {
  @ffi.Int()
  external int a;
}

const int BASE_MACRO_1 = 1;
