...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    output: Output(
      dart: DartOutput(
        path: testDir.resolve('block_annotation_test_bindings.dart'),
      ),
      objectiveCFile: testDir.resolve('block_annotation_test_bindings.m'),
      style: const NativeExternalBindings(assetId: 'package:ffigen/objc_test'),
    ),
    input: Input(entryPoints: [testDir.resolve('block_annotation_test.h')]),
