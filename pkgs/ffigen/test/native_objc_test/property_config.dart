...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    output: Output(
      dart: DartOutput(path: testDir.resolve('property_test_bindings.dart')),
      style: const NativeExternalBindings(assetId: 'package:ffigen/objc_test'),
    ),
    input: Input(entryPoints: [testDir.resolve('property_test.h')]),
    objectiveC: const ObjectiveC(),
    visitors: [
      Visitor(
        objCInterface: (node) {
          if (node.originalName == 'PropertyInterface') {
