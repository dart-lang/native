...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    output: Output(
      dart: DartOutput(path: testDir.resolve('native_objc_test_bindings.dart')),
      style: const NativeExternalBindings(assetId: 'package:ffigen/objc_test'),
    ),
    input: Input(entryPoints: [testDir.resolve('native_objc_test.m')]),
    objectiveC: const ObjectiveC(),
    visitors: [
      Visitor(
        objCInterface: (node) {
          node.isIncluded = node.originalName == 'Foo';
        },
      ),
    ],
  );
