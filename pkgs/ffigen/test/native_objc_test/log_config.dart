...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    output: Output(
      dart: DartOutput(path: testDir.resolve('log_test_bindings.dart')),
      style: const NativeExternalBindings(assetId: 'package:ffigen/objc_test'),
    ),
    input: Input(entryPoints: [testDir.resolve('log_test.m')]),
    objectiveC: const ObjectiveC(),
    visitors: [
      Visitor(
        objCInterface: (node) {
          const include = {'LogSpamBaseClass', 'LogSpamChildClass'};
