...
FfiGenerator getConfig([Uri? packageRoot]) {
  ...
  return FfiGenerator(
    output: Output(
      dart: DartOutput(path: testDir.resolve('rename_test_bindings.dart')),
      style: const NativeExternalBindings(assetId: 'package:ffigen/objc_test'),
    ),
    input: Input(entryPoints: [testDir.resolve('rename_test.m')]),
    ...
    visitors: [
      Visitor(
        ...
        objCMethod: (node) {
          ...
          if (parent is ObjCInterface && parent.originalName == '_Renamed') {
            if (node.selector == 'renamedMethod:otherArg:') {
              node.name = 'fooBarBaz';
            } else if (RegExp(r'ren.*P.*rty').hasMatch(node.selector)) {
              node.name = 'reProp';
            }
          }
        },
