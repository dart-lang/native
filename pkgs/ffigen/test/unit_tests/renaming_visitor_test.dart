...
import 'package:ffigen/src/header_parser/parser.dart' show transformBindings;
...
''')
    ...
    test(
      'toPublicAstNode maps each binding to its corresponding public AST node',
      () {
        final context = testContext(
          FfiGenerator(output: Output(dartFile: Uri.file('out.dart'))),
        );
        ...
        expect(
          Func(name: 'foo', returnType: voidType).toPublicAstNode(),
          isA<public_ast.Func>(),
        );
        expect(
          Struct(
        ...
        expect(
          ...
          isA<public_ast.EnumClass>(),
        );
        expect(
          Global(name: 'g', originalName: 'g', type: intType).toPublicAstNode(),
          isA<public_ast.Global>(),
        );
        expect(
          MacroConstant(
            name: 'm',
          ...
          isA<public_ast.MacroConstant>(),
        );
        expect(
          Typealias(name: 't', type: intType).toPublicAstNode(),
          isA<public_ast.Typealias>(),
        );
        expect(
          objcInterface.toPublicAstNode(),
          isA<public_ast.ObjCInterface>(),
        );
        expect(
          ObjCProtocol(
            usr: 'c_proto_usr',
        ...
        expect(
          ...
          isA<public_ast.UnnamedEnumConstant>(),
        );

        // Anonymous enum returns null.
        expect(
          EnumClass(
            name: 'e',
            originalName: 'e',
        ...
        // Internal bindings still map to public AST nodes via
        // toPublicAstNode().
        expect(
          Func(
            name: 'foo',
    ...
    test('parser.dart filters internal bindings from AST visitor nodes', () {
      final visitedNames = <String>[];
      final visitor = public_ast.Visitor(
        func: (n) => visitedNames.add(n.name),
        typealias: (n) => visitedNames.add(n.name),
        objCInterface: (n) => visitedNames.add(n.name),
        struct: (n) => visitedNames.add(n.name),
      );
      ...
      final publicFunc = Func(name: 'publicFunc', returnType: voidType);
      final internalFunc = Func(
        name: 'internalFunc',
        returnType: voidType,
        isInternal: true,
      );
      final publicTypealias = Typealias(name: 'PublicAlias', type: intType);
      ...
      expect(
        visitedNames,
        containsAll(['publicFunc', 'PublicAlias', 'PublicItf', 'PublicStruct']),
      );
      expect(visitedNames, isNot(contains('internalFunc')));
      expect(visitedNames, isNot(contains('InternalAlias')));
      expect(visitedNames, isNot(contains('InternalItf')));
    });
  });
}
