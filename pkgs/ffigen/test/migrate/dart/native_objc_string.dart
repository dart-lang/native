...
FfiGenerator getConfig({Uri? outputDir, Uri? packageRoot}) {
  ...
  final dartPath = outputDir != null
      ? outputDir.resolve('string_test_bindings.dart')
      : configDir.resolve('string_test_bindings.dart');
  final objcPath = outputDir != null
      ? outputDir.resolve('string_test_bindings.dart.m')
      : configDir.resolve('string_test_bindings.dart.m');
  return FfiGenerator(
    output: Output(
    ...
    input: Input(entryPoints: [configDir.resolve('string_test.m')]),
    objectiveC: const ObjectiveC(),
    visitors: [
      Visitor(
        objCInterface: (node) =>
            node.isIncluded = node.originalName == 'StringUtil',
      ),
    ],
  );
}
