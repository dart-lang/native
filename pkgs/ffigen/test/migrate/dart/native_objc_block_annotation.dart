...
FfiGenerator getConfig({Uri? outputDir, Uri? packageRoot}) {
  ...
  final dartPath = outputDir != null
      ? outputDir.resolve('block_annotation_test_bindings.dart')
      : configDir.resolve('block_annotation_test_bindings.dart');
  final objcPath = outputDir != null
      ? outputDir.resolve('block_annotation_test_bindings.m')
      : configDir.resolve('block_annotation_test_bindings.m');
  return FfiGenerator(
    output: Output(
    ...
    input: Input(entryPoints: [configDir.resolve('block_annotation_test.h')]),
    ...
    visitors: [
      Visitor(
        ...
        typealias: (node) =>
            node.isIncluded = (node.originalName == 'EmptyBlock')
        ...
        objCProtocol: (node) => node.isIncluded =
            node.originalName == 'BlockAnnotationTestProtocol',
      ),
    ],
  );
}
