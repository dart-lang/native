...
FfiGenerator getConfig({Uri? outputDir, Uri? packageRoot}) {
  ...
  final dartPath = outputDir != null
      ? outputDir.resolve('typedef_test_bindings.dart')
      : configDir.resolve('typedef_test_bindings.dart');
  final objcPath = outputDir != null
      ? outputDir.resolve('typedef_test_bindings.dart.m')
      : configDir.resolve('typedef_test_bindings.dart.m');
  return FfiGenerator(
    output: Output(
    ...
    input: Input(entryPoints: [configDir.resolve('typedef_test.m')]),
    objectiveC: const ObjectiveC(),
    visitors: [
      Visitor(
        typealias: (node) =>
            node.isIncluded = (node.originalName == 'SomeClassPtr')
