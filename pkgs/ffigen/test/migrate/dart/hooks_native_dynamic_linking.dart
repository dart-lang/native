...
FfiGenerator getConfig({Uri? outputDir, Uri? packageRoot}) {
  packageRoot ??= Platform.script.resolve(
    '../../../../hooks/example/build/native_dynamic_linking/',
  );
  final dartPath = outputDir != null
      ? outputDir.resolve('add.dart')
      : packageRoot.resolve('lib/add.dart');
  return FfiGenerator(
    output: Output(
...
''',
    ...
    input: Input(
      entryPoints: [packageRoot.resolve('src/add.h')],
      include: (uri) => Glob('/**/src/add.h').matches(uri.path),
    ),
    visitors: [
      Visitor(
