...
FfiGenerator getConfig({Uri? outputDir, Uri? packageRoot}) {
  ...
  final dartPath = outputDir != null
      ? outputDir.resolve('global_test_bindings.dart')
      : configDir.resolve('global_test_bindings.dart');
  final objcPath = outputDir != null
      ? outputDir.resolve('global_test_bindings.dart.m')
      : configDir.resolve('global_test_bindings.dart.m');
  return FfiGenerator(
    output: Output(
      dart: DartOutput(path: dartPath),
      objectiveCFile: objcPath,
      style: const DynamicLibraryBindings(
        wrapperName: 'GlobalTestObjCLibrary',
        wrapperDocComment: 'Tests global variables',
      ),
    ),
    input: Input(entryPoints: [configDir.resolve('global_test.h')]),
