...
void main() async {
  ...
  test('link hook caching', timeout: longTimeout, () async {
  ...
  test(
    'link hook cache isolate by entry-point and compiler',
    timeout: longTimeout,
    () async {
      await inTempDir((tempUri) async {
        await copyTestProjects(targetUri: tempUri);
        ...
        final treeshakingFile = File.fromUri(
          tempUri.resolve('treeshaking.json'),
        );
        ...
        // Run 5: (Entrypoint 1, compiler 'dart2js') -> Cache Hit! (matches
        // Run 2)
        await runLink([entry1], 'dart2js');
        expect(
          logMessages.join('\n'),
