...
This example demonstrates how JNIgen can resolve and generate bindings for 3rd-party Maven dependencies (like GSON and OkHttp) in a Groovy-based Android project without requiring a full build.
...
```bash
dart run tool/generate_bindings.dart # run from maven_libs_groovy project root
```
...
- **Groovy DSL Support:** Uses traditional `build.gradle` files for both the plugin and the example app.
