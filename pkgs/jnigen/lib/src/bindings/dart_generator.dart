...
''');
    ...
    void generateTypeClassGetter({required bool isNullable}) {
      ...
      final typeClassName = isNullable
          ? node.nullableTypeClassName
          : node.typeClassName;
      final typeClassGetterName = isNullable
          ? 'nullableType'
          : staticTypeGetter;
...
class _TypeClassGenerator extends TypeVisitor<_TypeClass> {
  ...
  _TypeClass visitDeclaredType(DeclaredType node) {
    ...
    final allTypeClasses = node.mapTypeParameters((isNullable, definedType) {
      return definedType.accept(
        _TypeClassGenerator(
          resolver,
          isConst: false,
    ...
    final typeArgsList = node.mapTypeParameters((isNullable, definedType) {
      return definedType.accept(
        _TypeGenerator(
          resolver,
          forInterfaceImplementation: forInterfaceImplementation,
...
''');
      ...
      if (node.isAsyncVoid) {
        ...
        final returningTypeClass = asyncReturnType
            .accept(_TypeClassGenerator(resolver))
            .name;
        s.write('''
