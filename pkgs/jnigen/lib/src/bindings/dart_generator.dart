...
class DartGenerator extends Visitor<Classes, Future<void>> {
  ...
  static const autoGeneratedNotice =
      '// AUTO GENERATED BY JNIGEN $version. '
      'DO NOT EDIT!\n';
...
${modifier}final $classRef = $_jni.JClass.forName(r'$internalName');
  ...
  void visit(ClassDecl node) {
    ...
    final interfaces = node.interfaces.map(
      (interface) =>
          interface.accept(_TypeGenerator(resolver, includeNullability: false)),
    );
...
class _TypeGenerator extends TypeVisitor<String> {
  ...
  String visitArrayType(ArrayType node) {
    ...
    final nullable = includeNullability && node.isNullable && isTopTypeNullable
        ? '?'
        : '';
  ...
  String visitDeclaredType(DeclaredType node) {
    ...
    final allTypeParams = node.mapTypeParameters((isNullable, definedType) {
    ...
    final typeParams = typeErasure
        ? ''
        : allTypeParams.join(', ').encloseIfNotEmpty('<', '>');
  ...
  String visitNonPrimitiveType(ReferredType node) {
    final nullable = includeNullability && isTopTypeNullable && node.isNullable
        ? '?'
        : '';
    return '$_jObject$nullable';
  }
  ...
  String getter(Field node) {
    ...
    if (node.type is PrimitiveType || node.type is ArrayType) {
      ...
      final type = node.type.accept(
        _TypeGenerator(resolver, typeErasure: true, includeNullability: false),
      );
      typeClass = '$type.type';
    }
...
''');
    ...
    final ifStatic = (node.isStatic || node.isConstructor) && !isTopLevel
        ? 'static '
        : '';
...
''');
      ...
      if (node.isAsyncVoid) {
        ...
        final returningType = asyncReturnType.accept(
          _TypeGenerator(resolver, includeNullability: false),
        );
        final returningTypeNullable = asyncReturnType.accept(
          _TypeGenerator(resolver),
        );
        final returningTypeErased = asyncReturnType.accept(
          _TypeGenerator(
            resolver,
        ...
        final returningTypeClass = asyncReturnType.accept(
          _TypeClassGenerator(resolver),
        );
...
''');
  ...
  void _writeKotlinSignature(Method node) {
    ...
    final typeParams = [...node.classDecl.allTypeParams, ...node.typeParams];
    ...
    if (kotlinFunction.typeParameters.isNotEmpty) {
      ...
      for (final t in kotlinFunction.typeParameters) {
        final typeParameters = t.upperBounds.map(
          (bound) => '${t.name} : ${bound.toDocComment(typeParams)}',
        );
    ...
    s.writeAll(
      kotlinFunction.valueParameters.map((p) {
...
class _ConcreteImplMethod extends Visitor<Method, void> {
  ...
  void visit(Method node) {
    ...
    final argsDef = node.paramsMaybeAsync
        .accept(_ParamDef(resolver, methodGenericErasure: true))
        .join(', ');
    final argsCall = node.paramsMaybeAsync
        .map((param) => param.finalName)
        .join(', ');
    s.write('''
...
''');
    ...
    if (node.isSuspendFun) {
      final resume = node.isAsyncVoid
          ? 'resumeWithVoidFuture'
          : 'resumeWithFuture';
      final contArg = StringBuffer();
      node.params.last.accept(
        _InterfaceParamCast(
          resolver,
          contArg,
