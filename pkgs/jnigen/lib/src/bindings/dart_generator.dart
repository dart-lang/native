...
String? _mavenJavadocUrl(JniGenerator config) {
...
class DartGenerator extends Visitor<Classes, Future<void>> {
  ...
  late final String? mavenJavadocUrl = _mavenJavadocUrl(config);
  ...
  Future<void> visit(Classes node) async {
    ...
    if (config.output.dart.structure == OutputStructure.singleFile) {
      ...
      final classGenerator =
          _ClassGenerator(config, s, resolver, mavenJavadocUrl);
...
A file that was not generated by JNIgen was found in ${root.path}: ${file.path}.''');
    ...
    for (var package in packages.keys) {
      ...
      final exports = (packages[package]!.toList()..sort())
...
class _ClassGenerator extends Visitor<ClassDecl, void> {
  ...
  final StringSink s;
  final Resolver resolver;
  final String? mavenJavadocUrl;

  _ClassGenerator(
    this.config,
    this.s,
    this.resolver,
    this.mavenJavadocUrl,
  );
  ...
  void generateFieldsAndMethods(
    ...
    final staticMethodGenerator = _MethodGenerator(
      ...
      resolver,
      staticSink,
      mavenJavadocUrl: mavenJavadocUrl,
      isTopLevel: node.isTopLevel,
      classRef: classRef,
    );
...
class _MethodGenerator extends Visitor<Method, void> {
  ...
  const _MethodGenerator(
    ...
    this.resolver,
    this.s, {
    required this.mavenJavadocUrl,
    required this.isTopLevel,
    required this.classRef,
  });

  String? _javadocUrl(Method node) {
    final baseUrl = mavenJavadocUrl;
    if (baseUrl == null) {
      return null;
    }

    final classDecl = node.classDecl;
    final packageName = classDecl.packageName;
    final packagePath = packageName.replaceAll('.', '/');

    final binaryClassName = packageName.isEmpty
        ? classDecl.binaryName
        : classDecl.binaryName.substring(packageName.length + 1);

    final className = binaryClassName.replaceAll(r'$', '.');

    final parameterTypes = node.params
        .map((param) => param.type.name.replaceAll(r'$', '.'))
        .join(',');

    final methodName =
        node.isConstructor ? className.split('.').last : node.name;

    final classUrl = packagePath.isEmpty
        ? '$baseUrl/$className.html'
        : '$baseUrl/$packagePath/$className.html';

    return '$classUrl#$methodName($parameterTypes)';
  }
